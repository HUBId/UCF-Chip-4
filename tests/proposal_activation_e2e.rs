...
fn proposal_evidence_payload(
    ...
    evidence.encode().expect("proposal payload")
}
...
fn proposal_activation_payload(
    ...
    let activation_digest = evidence.compute_digest().expect("activation digest");
    evidence.activation_digest = activation_digest;
    let payload = evidence.encode().expect("activation payload");
...
fn run_sequence() -> (String, Vec<String>) {
    ...
    assert_eq!(latest_activation.status, ActivationStatus::Applied);
...
fn proposal_activation_e2e_smoke_is_deterministic() {
    ...
    assert_eq!(first_reason_codes.len(), MAX_REASON_CODES);
}
